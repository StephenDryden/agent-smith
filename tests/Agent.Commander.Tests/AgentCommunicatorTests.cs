...
namespace Agent.Commander.Tests
{
    public class AgentCommunicatorTests
    {
        ...
        public async Task QueryToolsAsync_ShouldReturnTools_WhenAgentResponds()
        {
            ...
            // Act
            var tools = await communicator.QueryToolsAsync(agent);
        ...
        [Fact]
        public async Task QueryToolsAsync_ShouldHandleError_WhenAgentFails()
        {
            // Arrange
            var mockHttpClient = new Mock<HttpClient>();
            var communicator = new AgentCommunicator(mockHttpClient.Object);
            ...
            // Assert
            Assert.NotNull(tools);
            Assert.Contains("Error fetching tools", tools);
        }
    }
}
